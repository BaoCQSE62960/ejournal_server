    ...
    if (access.rows[0]) {
      if (access.rows[0].accesstype == sob.STUDENT) {
        ...
        const universityTran = await pool.query(
          `SELECT U.id, U.mailtype, UT.expirationdate AS expirationdate, UT.isexpired AS isexpired
        ...
        const personalTran = await pool.query(
          `SELECT * FROM  "personaltransaction" WHERE articleid = $1 AND accountid = $2`,
...
async function checkCorresponding(req, res, next) {
  try {
    const { id, title, summary, content, openaccess, majorid, authorlist } =
      req.body;
...
router.post("/submit/", checkRoleSubmit, async (req, res) => {
  try {
    const { title, summary, content, openaccess, majorid, authorlist } =
      req.body;
    var author = [];
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      var newManuscript = await pool.query(
        `INSERT INTO "article"(title,summary,content,doc,openaccess,creatorid,creationtime,status,majorid) 
    ...
    try {
      ...
      for (var x = 0; x < req.body.authorlist.length; x++) {
        var authorItem = await pool.query(
          ...
          WHERE fullname = $1 
          AND email = $2`,
          [req.body.authorlist[x].fullname, req.body.authorlist[x].email]
        );
...
router.delete("/manuscript/delete/", checkRoleAuthor, async (req, res) => {
  try {
    const { id } = req.body;
